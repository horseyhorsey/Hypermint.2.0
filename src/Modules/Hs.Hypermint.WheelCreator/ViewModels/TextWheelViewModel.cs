...
namespace Hs.Hypermint.WheelCreator.ViewModels
{
    ...
    private void GetPresets(string selectedName = "")
        ...
        {
            var settingsPath = Path.Combine(_settingsRepo.HypermintSettings.PresetPath, "wheel\\text\\");
            var name = Presets.CurrentItem?.ToString();
            if (name != null)
            {
                CurrentWheelSetting = _textImageService.DeserializePreset(settingsPath + name);
